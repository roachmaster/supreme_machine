...
read-huggingface-token() {
    ...
    if ! command -v pipx >/dev/null 2>&1; then
        ...
        sudo apt-get install -y pipx || { echo "❌ Failed to install pipx."; return 1; }
        export PATH="$HOME/.local/bin:$PATH"
    fi
    if ! command -v huggingface-cli >/dev/null 2>&1; then
        echo "📦 Installing huggingface-cli via pipx..."
        pipx install huggingface_hub || { echo "❌ Failed to install huggingface-cli via pipx."; return 1; }
    fi
}
...
hf-download-model() {
    local repo="$1" file="$2"
    [[ -z "$repo" || -z "$file" ]] && { echo "❌ Usage: hf-download-model <REPO> <FILENAME>"; return 1; }
    read-huggingface-token
    [[ -z "$HF_TOKEN" ]] && { echo "❌ HF_TOKEN not set."; return 1; }
    ...
    echo "⬇️  Downloading $file from $repo → $output..."
    curl -L -o "$output" -H "Authorization: Bearer $HF_TOKEN" "$url" || { echo "❌ Download failed"; return 1; }
...
docker-build-clean() {
    ...
    docker build --rm --force-rm -t "$image_name" -f "$dockerfile_path" "$context_dir" || { echo "❌ Docker build failed."; return 1; }
    clean-dangling-images
...
cuda-base-build() {
    ...
    docker build --rm --force-rm \
        ...
        -t "$CUDA_BASE_IMAGE" "$ROOT_DIR/docker/cuda" || { echo "❌ CUDA base build failed."; return 1; }
    clean-dangling-images
}